...
@st.cache_resource
def load_model():
    gdrive_id = "1_VCiUq6kWeLYUdaZjfu-ddA1wh0ObOzB"
    ...
    model = tf.keras.models.load_model(tmp_file.name, custom_objects={'KerasLayer': hub.KerasLayer})
    return model
...
def get_recipe(predicted_class, class_names):
    df = pd.read_csv('data_resep.csv', delimiter=';')
...
def get_nutrition(predicted_class, class_names):
    df = pd.read_csv('total_nutrition_per_food.csv')
    food_name = class_names[predicted_class].lower()
